...
class EvidenceCollection(BaseModel):
    ...
    supporting_evidence: List[Dict[str, Any]] = Field(default_factory=list, description="Supporting telemetry snippets")
...
class InvestigatorAgent(BaseAgent):
    ...
    async def execute(self, state: InvestigationSession, **kwargs: Any) -> Dict[str, Any]:
        ...
        # 1. Resolve fallback drift score
        drift_score = 0.25
        ...
        # 2. Structured LLM query attempt
        if self.llm_provider and hasattr(self.llm_provider, "generate_step_structured"):
            # Compile context telemetry dict
            telemetry_data = {
                "incident": state.incident.model_dump(mode="json"),
                "predictions": state.investigation_context.predictions if state.investigation_context else [],
            ...
            try:
                collection = await self.llm_provider.generate_step_structured(
                    ...
                    context={"context_json": telemetry_json},
                    temperature=0.1
                )
                ...
                if collection.supporting_evidence:
                    for ev in collection.supporting_evidence:
                        ...
                        drift_ev = DriftEvidence(
                            ...
                            summary=ev.get("summary", f"Covariate drift score of {score} observed on feature {feat}."),
                ...
                # Return original LLM collection dictionary for observability/archival
        ...
        return {"status": "EVIDENCE_COLLECTED", "evidence_id": fallback_ev.evidence_id}
